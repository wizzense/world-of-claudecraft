...
const hovered = await page.evaluate(() => {
  ...
  const row = rows.find((r) => /Rupture|Lacera|Кровопуск|割裂|파열|Ruptur/i.test(r.textContent || ''));
  if (!row) return null;
...
const result = await page.evaluate(() => {
  ...
  p.comboPoints = 5; p.comboTargetId = null;
  ...
  p.comboPoints = 5; p.comboTargetId = mob.id;
