...
import { t, setLanguage, ensureLocaleLoaded, formatNumber } from "../src/ui/i18n";
...
const EN: Record<(typeof HUD_CHROME_KEYS)[number], string> = {
  "hudChrome.tips.joinChannels": "Tip: type /join world or /join lfg to chat with players across the realm.",
...
describe("hudChrome.* keys (English-only catalog module)", () => {
  beforeAll(async () => { await ensureLocaleLoaded("es"); });
